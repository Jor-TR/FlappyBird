...
window.onload = () => {
                ...
                onStateChange.timerId = funcs.setSafeInterval(() => {
                    ...
                    if ((() => {
                        ...
                        if (
                            ...
                            //小鸟坠地
                            state.birdY > (config.skyHeight - config.birdSize.height) ||

                            //分数到达上限
                            state.score >= config.maxScore
    ...
    // let img=new Image();
    // img.onload=()=>{
    //     ctx.drawImage(img,config.titlePos.x,config.titlePos.y);
    // };
    // img.src=config.title.path;
};
